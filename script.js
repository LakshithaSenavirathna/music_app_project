...
const AppProvider = ({ children }) => {
    ...
    const playNext = () => {
        ...
        if (nextSong) {
            let newQueue = [...queue];
            if (repeatMode === 'all') {
               newQueue.push(currentSong); // add current song to the end
            }
            if(isShuffling) {
    ...
    const value = { data, screen, navigate, goBack, currentSong, isPlaying, currentTime, duration, volume, queue, audioRef, playSong, togglePlay, playNext, seekTo, setVolume, isShuffling, setIsShuffling, repeatMode, setRepeatMode, setQueue };

    return (
        <AppContext.Provider value={value}>
            {children}
            <audio ref={audioRef} onTimeUpdate={e => setCurrentTime(e.target.currentTime)} onLoadedMetadata={e => setDuration(e.target.duration)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={playNext} volume={volume} />
        </AppContext.Provider>
    );
};
...
const QueueModal = ({ onClose }) => {
     const { currentSong, queue, setQueue } = useContext(AppContext);
     return (
        <div className="queue-modal">
            ...
            <div className="queue-modal__content">
                 ...
                 {queue.length > 0 ? (
                    queue.map(song => <SongItem key={song.id} song={song} showArtwork={true} />)
                 ) : (
...
const NowPlayingScreen = () => {
    const { currentSong, isPlaying, currentTime, duration, togglePlay, playNext, goBack, seekTo, volume, setVolume, isShuffling, setIsShuffling, repeatMode, setRepeatMode } = useContext(AppContext);
    const [showVolume, setShowVolume] = useState(false);
    const [showQueue, setShowQueue] = useState(false);
    const progress = duration > 0 ? (currentTime / duration) * 100 : 0;
    const handleProgressClick = e => { const rect = e.currentTarget.getBoundingClientRect(); seekTo(((e.clientX - rect.left) / rect.width) * duration); };
        ...
        <div className="now-playing-screen">
            ...
            <div className="now-playing-screen__content">
                ...
                <div className="now-playing-screen__controls">
                    <button className="icon-btn" style={{color: isShuffling ? 'var(--color-primary)' : 'inherit'}} onClick={() => setIsShuffling(!isShuffling)}><i className="fa-solid fa-shuffle"></i></button>
                    <button className="icon-btn"><i className="fa-solid fa-backward-step"></i></button>
                    <button className="icon-btn play-btn" onClick={togglePlay}><i className={`fa-solid ${isPlaying ? 'fa-pause' : 'fa-play'}`}></i></button>
                    <button className="icon-btn" onClick={playNext}><i className="fa-solid fa-forward-step"></i></button>
                    <button className="icon-btn" style={{color: repeatMode !== 'none' ? 'var(--color-primary)' : 'inherit'}} onClick={toggleRepeat}><i className={`fa-solid ${repeatIcon()}`}></i></button>
                </div>
                <div className="now-playing-screen__bottom-actions">
                    ...
                    <button className="icon-btn" onClick={() => setShowVolume(!showVolume)}><i className="fa-solid fa-volume-high"></i></button>
                    <button className="icon-btn" onClick={() => setShowQueue(true)}><i className="fa-solid fa-list-ol"></i></button>
                </div>
            </div>
        </div>
    );
...
const SearchScreen = () => {
    return (
        <div className="screen">
            ...
            <input type="text" placeholder="What do you want to listen to?" style={{width: '100%', padding: 'var(--space-16)', border: 'none', borderRadius: 'var(--radius-base)', backgroundColor: 'var(--color-surface-raised)', color: 'var(--color-text)', fontSize: 'var(--font-size-base)'}} />
...
const LibraryScreen = () => {
    ...
    const [tab, setTab] = useState('playlists'); // playlists, artists, albums
    ...
    return (
        <div className="screen">
            <div className="screen-header">
                ...
                <div>
                     <button className="icon-btn" onClick={() => setView(v => v === 'grid' ? 'list' : 'grid')}><i className={`fa-solid ${view === 'grid' ? 'fa-list' : 'fa-grip'}`}></i></button>
            ...
            {tab === 'playlists' && <div style={{display: 'flex', flexDirection: 'column', gap: 'var(--space-8)'}}>{data.playlists.map(p => <SongItem key={p.id} song={{...p, artist: p.creator, artwork: p.artwork}} />)}</div>}
            {tab === 'artists' && <div style={{display: 'flex', flexDirection: 'column', gap: 'var(--space-8)'}}>{data.artists.map(a => <SongItem key={a.id} song={{...a, artist: a.genre}} />)}</div>}
            {tab === 'albums' && <div style={{display: 'flex', flexDirection: 'column', gap: 'var(--space-8)'}}>{data.albums.map(al => <SongItem key={al.id} song={{...al, title: al.name}} />)}</div>}
        </div>
    );
}
...
const DetailsScreen = ({ type, id }) => {
    ...
    if (type === 'playlist') {
        ...
        songs = item.songs.map(songId => data.songs.find(s => s.id === songId));
        title = item.name;
    ...
    return (
         <div className="screen" style={{padding: 0, paddingBottom: 'var(--content-padding-bottom)'}}>
            <div style={{position: 'absolute', top: 16, left: 16, zIndex: 10}}>
                 <button className="icon-btn" style={{backgroundColor: 'rgba(0,0,0,0.5)'}} onClick={goBack}><i className="fa-solid fa-arrow-left"></i></button>
            </div>
             ...
             <div style={{padding: '0 16px'}}>
                {songs.map(song => <SongItem key={song.id} song={song} onPlay={() => playSong(song, songs)} showArtwork={false} />)}
             </div>
         </div>
    );
};
    ...
    return (
        <div className="screen" style={{padding: 0, paddingBottom: 'var(--content-padding-bottom)'}}>
            ...
            <div style={{padding: 'var(--space-16)'}}>
                 ...
                 {artistSongs.map(song => <SongItem key={song.id} song={song} onPlay={() => playSong(song, artistSongs)} />)}
            </div>
        </div>
    );
};
